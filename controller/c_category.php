    ...
    if(isset($_GET['act'])){
        switch ($_GET['act']) {
            case 'detail':
                ...
                $dsHot = product_getHot(4);
                $ctDM= category_getById($_GET['id']);
                $dsIDM= category_getByIDM($_GET['id']);
                $dsSP = product_getCategory($_GET['id']);
                // hiển thị dữ liệu
            ...
            case 'addToCart':
                ...
                header("Location: $base_url?mod=product&act=detail&id=$MaSach");
            default:
                # code...
                break;
        }
